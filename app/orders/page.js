      ...
      <div className="max-w-md mx-auto mt-10 bg-white p-6 rounded-xl shadow-md flex-1">
        ...
        {orders.length === 0 ? (
          ...
          <div className="space-y-4">
            {orders.map((order) => (
              <div key={order.id} className="border p-6 rounded-lg shadow-md hover:shadow-lg transition-all">
                <div className="flex justify-between items-center">
                  ...
                  <div>
                    {/* Status Handling */}
                    <p className={`text-sm flex items-center ${order.payment_status === 'Success' ? 'text-green-500' : order.payment_status === 'Failed' ? 'text-red-500' : 'text-yellow-500'}`}>
